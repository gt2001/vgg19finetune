...
#    get_ipython().system('wget -O data/imagenet-vgg-verydeep-19.mat http://www.vlfeat.org/matconvnet/models/beta16/imagenet-vgg-verydeep-19.mat')
...
# ------------------------------------------------------------------- #
paths = {"image2/Positive_ID", "image2/Negative_ID", "image2/Unverified"}
...
def rgb2gray(rgb):
    if len(rgb.shape) is 3:
        ...
        return rgb
if use_gray:
    totalimg   = np.ndarray((imgcnt+11, imgsize[0]*imgsize[1]))
else:
    totalimg   = np.ndarray((imgcnt+11, imgsize[0]*imgsize[1]*3))
totallabel = np.ndarray((imgcnt+11, nclass))
paths = {"image2/Unprocessed"}
for relpath in paths:
    path = cwd + "/" + relpath
    ...
    for f in flist:
        ...
        if use_gray:
            ...
            grayimg  = currimg
        # Reshape
        graysmall = grayimg/255.
        grayvec   = np.reshape(graysmall, (1, -1))
        # Save
        totalimg[imgcnt, :] = grayvec
        totallabel[imgcnt, :] = np.eye(nclass, nclass)[1]
        print(fullpath," ", imgcnt)
        imgcnt    = imgcnt + 1
print("it's over_______________________________________")
imgcnt     = 0
paths = {"image2/Positive_ID", "image2/Negative_ID", "image2/Unverified"}
for i, relpath in zip(range(nclass), paths):
    ...
    for f in flist:
        ...
        print(i," ", fullpath," ", imgcnt)
        imgcnt    = imgcnt + 1

# Divide total data into training and test set
randidx    = np.random.randint(imgcnt, size=imgcnt)
randidx    = np.array(list(randidx) + list(range(imgcnt,imgcnt+11)))
print(randidx[1442]," ",randidx[1454])
trainidx   = randidx[0:int(4*imgcnt/5)]
testidx    = randidx[int(4*imgcnt/5):imgcnt+11] #这里有个+11
trainimg   = totalimg[trainidx, :]
trainlabel = totallabel[trainidx, :]
testimg    = totalimg[testidx, :]
testlabel  = totallabel[testidx, :]
print(randidx[int(4*imgcnt/5):imgcnt+11])
# testimg = np.array(list(testimg) + [0]*11)
# testlabel = np.array(list(testimg) + [0]*11)
...
test_pred=sess.run(_pred, feed_dict={x: test_vectorized,y: testlabel,keepratio:1.})
a1=tf.argmax(test_pred,1)
a2=tf.argmax(testlabel,1)
#print(logistic.cdf(test_pred))
for x in range(300):
    print(1.1**test_pred[x, :]/np.sum(1.1**test_pred[x, :]))
a=sess.run(a1)
b=sess.run(a2)
print(a)
print(b)